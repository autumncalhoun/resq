    ...
    if (inquiryThreads) {
      for (const inquiry of inquiryThreads) {
        ...
        threads.push({
          ...
          unread_count: 0,
          dog_name: inquiry.dogs[0]?.name || 'Unknown',
          dog_photos: inquiry.dogs[0]?.photos || '',
          organization_name:
            inquiry.dogs[0]?.organizations[0]?.name || 'Unknown',
      ...
      for (const application of applicationThreads) {
        ...
        threads.push({
          ...
          latest_message_time:
            latestMessage?.created_at || new Date().toISOString(),
          unread_count: 0,
          dog_name: application.dogs[0]?.name || 'Unknown',
          dog_photos: application.dogs[0]?.photos || '',
          organization_name:
            application.dogs[0]?.organizations[0]?.name || 'Unknown',
        ...
        <div className="px-4 py-6 sm:px-0">
          {threads.length === 0 ? (
            ...
            <div className="space-y-4">
              {threads.map((thread) => {
                const photos = thread.dog_photos
                  ? JSON.parse(thread.dog_photos)
                  : []
